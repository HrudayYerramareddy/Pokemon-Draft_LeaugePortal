...
public class LeagueFlowController {
    ...
    public record FaabTradeRequest(Long recipientTeamId,List<Long> offeredPokemonIds,List<Long> requestedPokemonIds,Integer proposerFaab,Integer recipientFaab){}
    @PostMapping("/trades") public TradeOffer trade(@RequestBody FaabTradeRequest r,HttpSession session){AppUser u=auth.current(session);if(u.getRole()==Role.MANAGER)throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Manager cannot offer trade");return freeAgency.offerTrade(u.getTeamId(),r.recipientTeamId(),r.offeredPokemonIds(),r.requestedPokemonIds(),r.proposerFaab()==null?0:r.proposerFaab(),r.recipientFaab()==null?0:r.recipientFaab());}
