...
public class AuthService {
    ...
    public AppUser login(String username,String password,HttpSession session){
        AppUser user=users.findByUsername(username).orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED,"Invalid login"));
        if(!user.getPassword().equals(password)) throw new ResponseStatusException(HttpStatus.UNAUTHORIZED,"Invalid login");
    ...
    public AppUser current(HttpSession session){
        ...
        return users.findById((Long)id).orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED,"Session expired"));
    }
    public AppUser manager(HttpSession session){
        AppUser u=current(session);
        if(u.getRole()!=Role.MANAGER) throw new ResponseStatusException(HttpStatus.FORBIDDEN,"Manager only");
        return u;
    }
    public Long teamId(HttpSession session){
        AppUser u=current(session);
        if(u.getRole()==Role.MANAGER) return null;
        if(u.getTeamId()==null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"No team assigned");
        return u.getTeamId();
    }
}
