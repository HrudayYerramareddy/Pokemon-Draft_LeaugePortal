...
public class ScheduleService {
  ...
  public ScheduleService(
      TeamRepository teams, MatchupRepository matchups, LeagueSettingsRepository settingsRepo) {
    this.teams = teams;
  ...
  public void generate() {
    ...
    if (a.size() != 8 || b.size() != 8)
      throw new IllegalStateException("Schedule requires exactly 8 teams in each division");
    Comparator<Team> order =
        Comparator.comparingInt(Team::getDraftPosition).thenComparing(Team::getId);
    a.sort(order);
    b.sort(order);
    matchups.deleteAll();
    addRoundRobin(a, 1);
  ...
  public void recalculateStandings() {
    ...
    for (Matchup m : matchups.findAll()) {
      ...
      h.setDifferential(h.getDifferential() + diff);
      a.setDifferential(a.getDifferential() - diff);
      if (diff > 0) {
        h.setWins(h.getWins() + 1);
        a.setLosses(a.getLosses() + 1);
      } else if (diff < 0) {
        a.setWins(a.getWins() + 1);
        h.setLosses(h.getLosses() + 1);
      }
    }
    teams.saveAll(all);
  }
  ...
  private Map<String, Integer> headToHead() {
    ...
    for (Matchup m : matchups.findAll()) {
      if (!m.isPlayed()
          || m.getHomeScore() == null
          || m.getAwayScore() == null
          || m.getHomeScore().equals(m.getAwayScore())) continue;
      ...
      h.put(key(winner, loser), 1);
      h.put(key(loser, winner), -1);
    }
    return h;
  }
