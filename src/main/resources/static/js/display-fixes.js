...
(() => {
  ...
  function bindDisplayCards(root, source) {
    ...
    root.querySelectorAll('.display-pokemon-card').forEach(card=>card.addEventListener('click',()=>{
      const id=Number(card.dataset.pokemonId),p=source.find(x=>Number(x.pokemonId??x.id)===id);
  ...
  window.renderRosters=async function(){
    ...
    $('#page-rosters').innerHTML=`<div class="section-title"><div><h1>Rosters</h1><p class="muted">10 Pokémon maximum · $100 roster value cap.</p></div></div><div class="grid roster-team-grid">${data.map(r=>{const value=r.pokemon.reduce((sum,p)=>sum+Number(p.price||0),0);return `<div class="card team-card roster-value-card"><div class="section-title"><div><h3>${esc(r.team.name)}</h3><p>${esc(r.team.coachName)} · Division ${r.team.division}</p></div><div class="roster-badges"><span class="pool-count">${r.pokemon.length}/10</span><span class="roster-value ${value>100?'over':''}">$${value}/$100</span></div></div><div class="roster-mini-grid">${r.pokemon.map(p=>displayCard(p)).join('')||'<p class="muted">No Pokémon rostered.</p>'}</div></div>`}).join('')}</div>`;
    const all=data.flatMap(r=>r.pokemon);bindDisplayCards($('#page-rosters'),all);
  };

  window.renderLineup=async function(){
    if(state.user.role==='MANAGER'){$('#page-lineup').innerHTML='<h1>Weekly Lineup</h1><div class="card"><p>Coach lineups appear on Schedule as soon as both teams submit.</p></div>';return;}
    const d=await api('/api/flow/lineup'),selected=new Set(d.selectedPokemonIds||[]);
    ...
    const grid=$('#lineup-grid');hydratePokemonCards(grid);
    grid.querySelectorAll('.display-selectable').forEach(c=>c.onclick=e=>{if(d.locked){const p=d.roster.find(x=>x.pokemonId===Number(c.dataset.pokemonId));if(p)showPokemonDetails(p);return;}const id=Number(c.dataset.pokemonId);if(selected.has(id)){selected.delete(id);c.classList.remove('selected')}else{if(selected.size>=6)return toast('You can only select 6 Pokémon',true);selected.add(id);c.classList.add('selected')}$('#lineup-count').textContent=`${selected.size}/6 selected`;$('#lineup-message').textContent=selected.size===6?'Ready to submit.':'Select exactly six Pokémon.';};
    grid.querySelectorAll('.display-selectable').forEach(c=>c.addEventListener('dblclick',e=>{e.preventDefault();const p=d.roster.find(x=>x.pokemonId===Number(c.dataset.pokemonId));if(p)showPokemonDetails(p)}));
    $('#submit-lineup').onclick=async()=>{if(selected.size!==6)return toast('Select exactly 6 Pokémon',true);try{await api('/api/flow/lineup',{method:'POST',body:{pokemonIds:[...selected]}});toast('Weekly lineup submitted');await renderLineup()}catch(e){toast(e.message,true)}};
  };

  window.renderSchedule=async function(){
    const data=await api('/api/flow/schedule'),by={};data.forEach(m=>(by[m.week]??=[]).push(m));
